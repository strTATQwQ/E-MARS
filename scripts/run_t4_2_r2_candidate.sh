...
TAG="${1:?usage: run_t4_2_r2_candidate.sh 0p35|0p32|0p30 ATTEMPT}"
ATTEMPT="${2:?usage: run_t4_2_r2_candidate.sh 0p35|0p32|0p30 ATTEMPT}"
case "$TAG" in
  0p35|0p32|0p30) ;;
  *) echo "invalid inflation tag: $TAG" >&2; exit 2 ;;
esac

RUN_LABEL="t4_2_r2_inflation_${TAG}_smoke"
DATASET="$CONTROL_ROOT/episodes/t4_2_obstacle_smoke_one"
CONFIG="$CONTROL_ROOT/configs/internnav_t4/t4_2_r2_inflation_${TAG}.yaml"
...
nohup setsid env \
  ...
  INTERNVLA_T4_NAV2_PARAMS_OVERRIDE="$CONFIG" \
