#!/usr/bin/env python3
"""Fail-closed T0.2 launcher for FastPolicy + oracle Slow + Nav2."""
...
def main() -> int:
    ...
    parser.add_argument("--output", default="results/omninav_t0/t0_2_fast_oracle_slow")
    parser.add_argument("--episode-backend", default="omninav_nav2.ros_runtime:FastOracleSlowBackend")
    parser.add_argument("--manifest", default="")
    parser.add_argument("--phase", choices=("canary", "pilot"), default="canary")
    args = parser.parse_args()
    try:
        require_passed_gate(args.gate_status, "T0.1")
    except GateBlocked as exc:
        path = write_blocked_result(args.output, gate="T0.2", required_gate="T0.1", reason=str(exc))
        print(f"T0.2 NOT RUN: {exc}; evidence={path}", file=sys.stderr)
        return 3
    if not args.manifest or not Path(args.manifest).is_file():
        raise SystemExit("a frozen OVON oracle-frontier manifest is required")
    ...
    try:
        ...
        raise SystemExit(f"real Fast/Nav2 backend is unavailable: {exc}") from exc
