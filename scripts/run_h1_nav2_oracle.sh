...
RESULT_DIR="${INTERNVLA_ORACLE_RESULT_DIR:-$CONTROL_ROOT/results/t1_3_h1_nav2_oracle}"
PARAMS="${INTERNVLA_NAV2_PARAMS:-$CONTROL_ROOT/configs/internnav_t1_t2/nav2_active_rolling.yaml}"
CONFIG="${INTERNVLA_ORACLE_CONFIG:-$CONTROL_ROOT/configs/internnav_t1_t2/h1_nav2_oracle_cfg.py}"
SCRIPT_ROOT="${INTERNVLA_SCRIPT_ROOT:-$CONTROL_ROOT/scripts}"
SOCKET_PATH="${INTERNVLA_ORACLE_SOCKET:-/tmp/internvla_oracle.sock}"
TASK_NAME="${INTERNVLA_ORACLE_TASK_NAME:-internnav_t1_h1_nav2_oracle_pass}"
...
nav2_pid=$!
setsid "$ROS_WS/install/internvla_nav2_adapter/lib/internvla_nav2_adapter/internvla_nav2_active" --ros-args \
...
env \
  ...
  "$ISAAC_PYTHON" "$SCRIPT_ROOT/run_internnav_nav2_oracle_entrypoint.py" \
...
length = int(metrics.get("length", metrics.get("Count", 0)))
passing = length == 10 and sr >= 0.9 and active.get("failure_count") == 0 and active.get("goal_count", 0) > 0
