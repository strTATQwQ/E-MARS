...
from isaac_vln_benchmark.v8_coverage_utils import postprocess_stop_coverage_run
from run_v8_route_oracle_coverage import common_live_args


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run V8 semantic stop coverage diagnostic.")
    ...
    out_dir = Path(args.output) if args.output else ROOT / "runs" / f"v8_semantic_stop_coverage_{stamp}"
    ...
    if not args.analyze_existing:
        cmd = [
            ...
            "--config",
            str(ROOT / "configs" / "v8_semantic_stop_coverage.yaml"),
            "--modes",
            "omninav_semantic_stop_coverage_v8",
            "--max-episodes",
    ...
    metrics = postprocess_stop_coverage_run(out_dir)
    summary = metrics.get("v8_semantic_stop_coverage", {})
    print(json.dumps({"output": str(out_dir), "semantic_stop_coverage": summary}, indent=2, ensure_ascii=False))
