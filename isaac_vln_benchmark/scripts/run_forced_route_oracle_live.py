...
from isaac_vln_benchmark.v4_live_probe import mock_route_run, run_route_live
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default=str(ROOT / "configs" / "forced_route_oracle_live.yaml"))
    ...
    args.output = Path(args.output) if args.output else ROOT / "runs" / f"forced_route_oracle_live_{stamp}"
    ...
    if args.remote_live:
        ...
        metrics = run_route_live(cfg, args.output, project_root=ROOT)
        print(json.dumps(metrics, indent=2, ensure_ascii=False))
        return 0 if metrics.get("pass") else 2
    if args.mock_models:
        cfg = load_yaml(config_path)
        metrics = mock_route_run(cfg, args.output)
        print(json.dumps(metrics, indent=2, ensure_ascii=False))
        return 0 if metrics.get("pass") else 2
    return run_remote_probe(args, script_name="run_forced_route_oracle_live.py", config_path=config_path)
