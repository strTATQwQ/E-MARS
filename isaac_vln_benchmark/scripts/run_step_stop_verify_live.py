...
from isaac_vln_benchmark.v4_live_probe import mock_step_stop_run, run_step_stop_live
...
def write_skipped(output: Path, reason: str) -> None:
    ...
    metrics = {"benchmark": "step_stop_verify_live", "status": "SKIPPED", "pass": False, "reason": reason}
    (output / "metrics.json").write_text(json.dumps(metrics, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (output / "summary.md").write_text(f"# step_stop_verify_live\n\n- status: SKIPPED\n- reason: {reason}\n", encoding="utf-8")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default=str(ROOT / "configs" / "step_stop_verify_live.yaml"))
    parser.add_argument("--forced-stop-result", default="")
    ...
    args.output = Path(args.output) if args.output else ROOT / "runs" / f"step_stop_verify_live_{stamp}"
    if not args.output.is_absolute():
        args.output = ROOT / args.output
    guard = load_metrics(args.forced_stop_result)
    if guard and not bool(guard.get("pass", False)):
        write_skipped(args.output, "forced stop oracle did not pass")
        return 2
    cfg = load_yaml(config_path)
    if args.remote_live:
        metrics = run_step_stop_live(cfg, args.output, project_root=ROOT)
        print(json.dumps(metrics, indent=2, ensure_ascii=False))
        return 0 if metrics.get("pass") else 2
    if args.mock_models:
        metrics = mock_step_stop_run(cfg, args.output)
        print(json.dumps(metrics, indent=2, ensure_ascii=False))
        return 0 if metrics.get("pass") else 2
    return run_remote_probe(
        args,
        script_name="run_step_stop_verify_live.py",
