"""Ten-episode official-task H1 Nav2 oracle overlay."""
...
spec = importlib.util.spec_from_file_location("internnav_h1_nav2_oracle_upstream", upstream)
...
if episode_count != 10:
    raise RuntimeError(f"oracle overlay must contain exactly 10 episodes, got {episode_count}")
...
eval_cfg.task.task_name = os.environ.get(
    "INTERNVLA_ORACLE_TASK_NAME", "internnav_t1_h1_nav2_oracle_pass"
)
eval_cfg.agent.model_settings["vis_debug"] = False
eval_cfg.agent.model_settings["vis_debug_path"] = (
    f"logs/internnav_t1/{eval_cfg.task.task_name}"
)
