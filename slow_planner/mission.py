"""Optional simulation mission-normalization contracts.

When the layer is enabled, the raw operator instruction terminates at the
selected Step model. Downstream navigation consumes only
:class:`CanonicalMission`. The simulation launcher may also leave the layer
disabled and preserve the frozen episode instruction unchanged.
"""
...
MISSION_ROUTE = "sim_instruction_normalization_v1"
...
class MissionNormalizationRequest:
    ...
    def identity(self) -> str:
        return f"sim::{self.mission_id}::{self.reset_generation}::{self.sequence_id}"
